...
export function useCourses(keyword: string, page: number, size = 10)
{
    ...
    const fetchCourses = useCallback(() => {
        ...
        getCourses(keyword, page, size)
            ...
            .catch((err) => {
                let message = 'Da xay ra loi khong xac dinh, vui long thulai.';

                if (axios.isAxiosError<ApiErrorResponse>(err)) {
                    if (err.response?.data?.message) {
                        message = err.response.data.message;
                    } else if (!err.response) {
// Khong nhan duoc response nao ca - Gateway hoaccourse-service dang tat

                        message = 'Khong ket noi duoc toi he thong. Vui long thulai sau.';
                    }
                }
